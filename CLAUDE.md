...
7. Do not push or commit to GitHub without the session runner's explicit go-ahead, and never treat a push itself as proof that a brief step has been reviewed. Pushing to enable a review (for example, so a collaborator can read the diff on GitHub) is a legitimate reason to push before sign-off, but the sign-off is a separate, later event that must still be confirmed explicitly, not inferred from the repo being up to date. As of 2026-08-30 the repo is connected to `kintaluk/toni` on GitHub (private) specifically so Step 6's read-through review can happen there; that push is not itself the sign-off.
